...
if MarineorJetpackMarineorExoRoll == 2 then
        ...
        */
         ...
         */
           ...
           self:NotifyMarine( nil, "%s is being hit by a Slap Bomb", true, Player:GetName())
            ...
            if not Player:GetIsAlive() then self:DestroyTimer(6) self.ScreenText.End(57) return end
      ...
      self:CreateTimer(7, 1, kNerveGasTimer, function () if not Player:GetIsAlive() then self:DestroyTimer(7) self.ScreenTextEnd(58) return end Player:GiveItem(GasGrenade.kMapName) end )
      return 
     ...
     Player:SetFuel(1)
     self:CreateTimer(8, 1, kInfiniteJetpackFuelDuration, function () if not Player:GetIsAlive() then self:DestroyTimer(8) self.ScreenText.End(53) return end CreateEntity(AmmoPack.kMapName, Player:GetOrigin(), Player:GetTeamNumber()) Player:ActivateNanoShield()  end )
     self:AddDelayToPlayer(Player) 
      return 
            ...
            self:CreateTimer(9, 4, healthpacksamount, function () if not Player:GetIsAlive() then self:DestroyTimer(9) self.ScreenText.End(53) return end CreateEntity(MedPack.kMapName, Player:GetOrigin(), Player:GetTeamNumber()) Player:ActivateNanoShield()  end )
            ...
            self:CreateTimer(10, 8, ammopacksamount, function () if not Player:GetIsAlive() then self:DestroyTimer(10) self.ScreenText.End(53) return end CreateEntity(AmmoPack.kMapName, Player:GetOrigin(), Player:GetTeamNumber()) Player:ActivateNanoShield()  end )
  ...
  //self:NotifyAlien( nil, "%s Player is an alive alien, not commander.", true, Player:GetName())
   ...
   local playermaxarmor = Player:GetMaxArmor()
     ...
     if AlienRoll == 2 then 
      ...
      self:CreateTimer(12, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(12) self.ScreenText.End(64) return end Player:GiveItem(EnzymeCloud.kMapName) end ) 
     ...
     Shine.ScreenText.Add( 61, {X = 0.20, Y = 0.80,Text = "Electrified: %s",Duration = kElectrifyTimer,R = math.random(0,255), G = math.random(0,255), B = math.random(0,255),Alignment = 0,Size = 1,FadeIn = 0,}, Player )
      ...
      self:CreateTimer(14, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(14) self.ScreenText.End(70) return end Player:GiveItem(HallucinationCloud.kMapName) end )
      ...
      self:CreateTimer(16, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(16) self.ScreenText.End(64) return end Player:GiveItem(MucousMembrane.kMapName) end ) 
      return 
      ...
      self:CreateTimer(17, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(17) self.ScreenText.End(64) return end Player:GiveItem(Contamination.kMapName) end ) 
      ...
      self:CreateTimer(18, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(18) self.ScreenText.End(64) return end Player:GiveItem(NutrientMist.kMapName) end ) 
      ...
      self:CreateTimer(19, 8, amount * 8, function () if not Player:GetIsAlive() then self:DestroyTimer(19) self.ScreenText.End(64) return end Player:GiveItem(ShadeInk.kMapName) end ) 
