...
local function GiveConstructOrder(who, where)
       ...
       local constructable =  GetNearestMixin(who:GetOrigin(), "Construct", 1, function(ent) return isPayload(ent:GetOrigin(), ent) and not ent:GetIsBuilt() and ent:GetCanConstruct(who) and who:CheckTarget(ent:GetOrigin()) and not (GetIsInSiege(ent) and not GetSiegeDoorOpen() )  end)
