...
function Welder:PerformWeld(player)
    ...
    if didHit and target and HasMixin(target, "Live") then
        ...
        if ( target.GetIsBuilt and target:GetIsBuilt() ) and player:GetTeamNumber() == target:GetTeamNumber() and HasMixin(target, "Weldable") and  HasMixin(target, "Levels") and target:GetLevel() ~= target:GetMaxLevel() and not target:isa("ARC") then
